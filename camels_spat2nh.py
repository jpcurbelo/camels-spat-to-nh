...
def camels_spat2nh_parallel(data_dir, data_gen):
    ...
    # # Process data for each basin and save to csv file
    # for country in data_gen['countries']:
    #     country_dir = os.path.join(data_dir_out, f'CAMELS_spat_{country}')
    #     if not os.path.exists(country_dir):
    #         os.makedirs(country_dir)
    ...
    #     for basin_f in basin_data_path_dict[country]:
    #         print('\n', basin_f)
    #         # Use ThreadPoolExecutor for parallel processing of each basin
    #         with ThreadPoolExecutor() as executor:
    #             # future = executor.submit(process_basin, data_dir, data_gen, country, basin_f)
    #             future = executor.submit(process_basin, data_gen, basin_f, data_sources, relative_path_forc, 
    #                                      relative_path_targ, basin_data_path, country_dir)
    #             future.result()  # Wait for the task to complete before moving to the next basin
...
def process_basin(data_gen, basin_f, data_sources, relative_path_forc, relative_path_targ, 
                  basin_data_path, country_dir):
